/**
 * Common Paper Mutual NDA Standard Terms, Version 1.0 — transcribed verbatim
 * from `templates/Mutual-NDA.md`, free to use under CC BY 4.0.
 *
 * The source marks every cover-page cross-reference with
 * `<span class="coverpage_link">…</span>`. Those become `{ ref }` segments here,
 * which is what lets the document light up the exact clauses a cover-page value
 * governs. `**bold**` runs in the source become `{ strong }` segments.
 */

import type { CoverPageField } from "./nda";
...
export const STANDARD_TERMS: StandardTermsSection[] = [
  {
    ...
    body: [
      "This Mutual Non-Disclosure Agreement (which incorporates these Standard Terms and the Cover Page (defined below)) (“",
      { strong: "MNDA" },
      "”) allows each party (“",
      { strong: "Disclosing Party" },
      ...
      { strong: "Receiving Party" },
      "”) as “confidential”, “proprietary”, or the like or (2) should be reasonably understood as confidential or proprietary due to its nature and the circumstances of its disclosure (“",
      ...
      { strong: "Cover Page" },
      "”). Each party is identified on the Cover Page and capitalized terms have the meanings given herein or on the Cover Page.",
    ],
  },
  {
